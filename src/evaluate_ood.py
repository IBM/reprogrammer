...
parser.add_argument('--alpha', default=0.4, type=float, help='weight of the zero-shot residual connection')
...
def get_rp_score(model, inputs, texts):
    with torch.no_grad():
...
def get_rrp_score(model, preprocess, inputs, texts, alpha):
    ...
    with torch.no_grad():
        zs_image_features, zs_text_features, _ = zero_shot(zs_inputs, texts)
        image_features, text_features, logit_scale = model(inputs, texts)
...
def get_score(args, model, preprocess, inputs, texts):
    if args.method == 'lp' or args.method == 'ft':
        scores = get_lp_score(model, inputs)
    elif args.method == 'rp':
        scores = get_rp_score(model, inputs, texts)
    elif args.method == 'resrp':
        scores = get_rrp_score(model, preprocess, inputs, texts, args.alpha)
...
def eval_ood_detector(args, directory, out_datasets):
    ...
    for i, (inputs, targets) in enumerate(ID_loader):
        ...
        scores = get_score(args, model, preprocess, inputs, texts)
        for score in scores:
            score_file.write("{}\n".format(score))
